...
|------|-------------|------|---------|:--------:|
| <a name="input_access_key_rotation_days"></a> [access\_key\_rotation\_days](#input\_access\_key\_rotation\_days) | Number of days before rotating the IAM access key (only applies when access\_method = 'user') | `number` | `90` | no |
...
| <a name="input_external_id"></a> [external\_id](#input\_external\_id) | Datadog external ID for the IAM assume role trust policy. Only required when upgrading existing integrations where the Datadog provider (v3.x+) has already cleared the value from state. Find the current value in the existing IAM role's trust policy in AWS. | `string` | `null` | no |
...
| <a name="input_log_forwarder_lambda_log_retention_days"></a> [log\_forwarder\_lambda\_log\_retention\_days](#input\_log\_forwarder\_lambda\_log\_retention\_days) | The number of days to retain logs for the log forwarder lambda | `number` | `90` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_external_id"></a> [external\_id](#output\_external\_id) | Datadog external ID used in the IAM assume role trust policy |
